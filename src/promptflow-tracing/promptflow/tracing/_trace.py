    ...
    def enrich(self, span, inputs, output, function, *args, **kwargs):
        ...
        if not IS_LEGACY_OPENAI:
            ...
            if isinstance(output, (Assistant)):
                ...
                tools = output.tools
            try:
                span.set_attribute("gen_ai.assistant.id", id) if id is not None else None
                span.set_attribute("gen_ai.assistant.model", model) if model is not None else None
                span.set_attribute("gen_ai.assistant.name", name) if name is not None else None
                span.set_attribute("gen_ai.assistant.instructions", instructions) if instructions is not None else None
                span.set_attribute("gen_ai.assistant.tools", serialize_attribute(tools)) if tools is not None else None
                if function.__name__ == "create":
                    span.add_event("gen_ai.assistant.created", {"gen_ai.assistant.id": id})
                elif function.__name__ == "update":
                    span.add_event("gen_ai.assistant.updated", {"gen_ai.assistant.id": id})
                elif function.__name__ == "delete":
                    span.add_event("gen_ai.assistant.deleted", {"gen_ai.assistant.id": id})
            except Exception as e:
    ...
    def enrich(self, span, inputs, output, function, *args, **kwargs):
        ...
        if not IS_LEGACY_OPENAI:
            ...
            if isinstance(output, (Thread)):
                ...
                try:
                    span.set_attribute("gen_ai.thread.id", id) if id is not None else None
                    span.set_attribute("gen_ai.thread.created_at", created_at) if created_at is not None else None
                    tool_resources = ""
                    if len(args[0]) > 0:
                        if len(args[0][0]) > 1:
                    ...
                    span.set_attribute(
                        "gen_ai.thread.tool_resources", serialize_attribute(tool_resources)
                    ) if tool_resources is not None else None
                    if function.__name__ == "create":
                        span.add_event("gen_ai.thread.created", {"gen_ai.thread.id": id})
                    elif function.__name__ == "update":
                        span.add_event("gen_ai.thread.updated", {"gen_ai.thread.id": id})
                    elif function.__name__ == "delete":
                        span.add_event("gen_ai.thread.deleted", {"gen_ai.thread.id": id})
                except Exception as e:
